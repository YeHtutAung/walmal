...
|----------|-------|--------------------|-------------|
| `GET /api/v1/product/search?q=&status=` | `walmal-product` (`ProductController` → `ProductSearchServiceImpl` → `ProductRepository.searchByNameBrandSkuOrBarcode`) | Case-insensitive *contains* on product name, brand, variant SKU, or variant barcode (widened from name/brand by the global-search feature). Optional `status` filter (2026-07-18): null = all statuses (admin list contract), `ACTIVE`/`INACTIVE` restricts; the blank-`q` path swaps `findAll` for a derived `findByStatus` | Hand-written JPQL (`LEFT JOIN p.variants`, `SELECT DISTINCT`, explicit `countQuery` — the derived count over a DISTINCT join over-counts joined rows; the `(:status IS NULL OR p.status = :status)` predicate appears in BOTH query strings, same over-count reason) |
...
| `store` | `ghcr.io/yehtutaung/walmal-store` | Next.js storefront; runtime `STRIPE_SECRET_KEY` (same value as `app`'s) for server-side PaymentIntent creation |
...
| `postgres`/`redis`/`rabbitmq`/`minio` | stock | unchanged, backend-only |

Backup: `deploy/backup.sh` (`pg_dump -Fc` + a tar of the MinIO data volume via
`docker compose exec`/a throwaway alpine container, 7-day rotation) — distinct
