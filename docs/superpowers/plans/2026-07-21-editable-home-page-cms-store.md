...
- [ ] **Step 2: `category-tiles.tsx`** — add prop `{ tiles?: HomeContent['categoryTiles'] }`. If `tiles?.length`, map over them (image `resolveMinioUrl(tile.imageUrl)`, `tile.label`, link `tile.href`); else render the current static 4-tile array. Keep the existing grid/markup; the dynamic branch must produce the same tile markup per item.
